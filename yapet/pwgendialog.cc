...
PWGenDialog::createWindow() throw(YAPET::UI::UIException) {
    ...
    if (window == NULL)
        throw YAPET::UI::UIException(_(
                                         "Error creating password generator window") );
...
PWGenDialog::printTitles() throw(YAPET::UI::UIException) {
    int retval =
        mymvwaddstr(window, 0, 2, _("P A S S W O R D  G E N E R A T O R") );

    if (retval == ERR)
        throw YAPET::UI::UIException(_("Error printing title") );

    char tmpbuff[1024];
    snprintf(tmpbuff, 1024, _("Generated Password (using %s):"),
             getNameOfRNG().c_str() );
...
PWGenDialog::PWGenDialog() throw(YAPET::UI::UIException) :
    ...
    YAPET::PWGEN::RNGENGINE requested_rng =
        YAPET::CONFIG::config.pwgen_rng.get();
...
PWGenDialog::run() throw(YAPET::UI::UIException) {
    ...
    while (true) {
        ...
        pwlen = pwleninput->getInt() >
                YAPET::CONSTS::Consts::getMaxPWLength() ?
...
RESTART_GENBUTTON:
        ...
        case KEY_ENTER:
                ...
                try {
                    msgbox =
                        new YAPET::UI::MessageBox(_("E R R O R"), _(
                                                      "You need to select at least one Character Pool")  );
                    msgbox->run();
                    delete msgbox;
            ...
            pwgen.setNewPool(CheckBoxOptions2Charpools(
                                 ckbxgroup->getOptions() ) );
            pwgen.generatePassword(pwlen);
            pwdisplay->clearText();
            pwdisplay->setText(pwgen.getPassword() );
