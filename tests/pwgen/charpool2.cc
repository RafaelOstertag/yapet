...
int main(int, char**) {
    ...
    try {
        ...
        if (tmp1.hadPoolReads(LETTERS)) return 1;
        if (tmp1.hadPoolReads(PUNCT)) return 1;
        if (tmp1.hadPoolReads(OTHER)) return 1;

        if (tmp1.getPoolsWithRead() != 0) return 1;
        if (tmp1.numPoolsNotRead() != 3) return 1;

        size_t start;
        size_t len;
        len = tmp1.getPoolPos(LETTERS, &start);
        if (len != std::strlen(t_letters)) return 1;
        if (tmp1[start] != t_letters[0]) return 1;
        if (tmp1[start + len - 1] != t_letters[std::strlen(t_letters) - 1])
            return 1;
        if (!tmp1.hadPoolReads(LETTERS)) return 1;
        if (tmp1.numPoolsNotRead() != 2) return 1;

        len = tmp1.getPoolPos(PUNCT, &start);
        if (len != std::strlen(t_punct)) return 1;
        if (tmp1[start] != t_punct[0]) return 1;
        if (tmp1[start + len - 1] != t_punct[std::strlen(t_punct) - 1])
            return 1;
        if (!tmp1.hadPoolReads(PUNCT)) return 1;
        if (tmp1.numPoolsNotRead() != 1) return 1;

        len = tmp1.getPoolPos(OTHER, &start);
        if (len != std::strlen(t_other)) return 1;
        if (tmp1[start] != t_other[0]) return 1;
        if (tmp1[start + len - 1] != t_other[std::strlen(t_other) - 1])
            return 1;
        if (!tmp1.hadPoolReads(OTHER)) return 1;
        if (tmp1.numPoolsNotRead() != 0) return 1;
