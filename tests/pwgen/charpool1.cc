...
class gainaccess : public CharacterPool {
   public:
    gainaccess(SUBPOOLS p) : CharacterPool(p) {}
    const char* _getPool() { return getPool(); }
};
...
int main(int, char**) {
    ...
    try {
        ...
        if ((tmp4.getPoolLength() != std::strlen(t_letters)) ||
            std::strncmp(tmp4._getPool(), t_letters, tmp4.getPoolLength()) !=
                0) {
        ...
        if ((tmp5.getPoolLength() != std::strlen(t_digits)) ||
            std::strncmp(tmp5._getPool(), t_digits, tmp5.getPoolLength()) !=
                0) {
        ...
        if ((tmp7.getPoolLength() != std::strlen(t_special)) ||
            std::strncmp(tmp7._getPool(), t_special, tmp7.getPoolLength()) !=
                0) {
        ...
        for (size_t i = 0; i < cp->getPoolLength(); i++) {
            ...
            if (c == 0) {
                std::cout << std::endl
                          << " ==> Zero encountered at position " << i
                          << std::endl;
                return 1;
            }
