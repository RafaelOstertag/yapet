...
class CharacterPool {
   ...
   protected:
    ...
    inline size_t pool_letters_len() const { return std::strlen(letters); }
    inline size_t pool_digits_len() const { return std::strlen(digits); }
    inline size_t pool_punct_len() const { return std::strlen(punct); }
    inline size_t pool_special_len() const { return std::strlen(special); }
    inline size_t pool_other_len() const { return std::strlen(other); }
    inline size_t pool_len(SUBPOOLS p) const {
        switch (p) {
            ...
            case ALL:
                return pool_letters_len() + pool_digits_len() +
                       pool_punct_len() + pool_special_len() + pool_other_len();
            default:
                assert(0);
                return -1;
        }
    ...
    inline const char* getPool(size_t* len) const {
        ...
        if (len == 0) throw std::runtime_error(_("0 pointer passed."));

#endif
        *len = pool_length;
        return pool;
    }
    inline const char* getPool() const noexcept { return pool; }

   public:
    explicit CharacterPool(int p);
    explicit CharacterPool(SUBPOOLS p);
    virtual ~CharacterPool() noexcept;
    ...
    inline int getAllocatedPools() const { return pools_allocated; }
     ...
     */
    inline int getPoolsWithRead() const { return pools_reads; }
    /**
     ...
     * read from.
     */
    inline void resetPoolsWithRead() { pools_reads = 0; }
    /**
    ...
    inline size_t getPoolLength() const noexcept { return pool_length; }

    //! Return a character at the position specified
    char operator[](size_t pos);

    inline size_t operator()() const noexcept { return getPoolLength(); }
